---
name: chaipai
description: 拆拍台只读陪拆。制作人谈到拆拍台、编排、拆拍、子阶段、拍、看做选、没想好、四格、泳道、起承转合、判决句、停车场，或者说「看看我在拆的」「我现在写到哪了」「给我拆拍的建议」「盯着我拆」时使用。只通过 chaipai CLI 读取，不直接读文件，不写任何数据。
---
<!-- 由 编排/build-skills.cjs 从 编排/skill/SKILL.src.md 生成，不要手改 -->
...
| 看整张网（分支、选项跳转、汇合、要先发生过） | `chaipai show 流向` |
| 看未定清单（他标了「没想好」、写了卡点的地方） | `chaipai show 未定` |
...
3. 未定清单里的东西是他自己还没想好的开放问题，不是缺陷：可以帮他想，不要替他定。
4. 页面可能已关时（brief/now 的「新鲜度」会说），要跟制作人说明你看的是哪个时间点的内容。
5. 不碰 `编排/` 下的任何文件，除非制作人明确要求改拆拍台这个工具本身。
